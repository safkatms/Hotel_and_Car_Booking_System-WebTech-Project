...
function CarBookingConfirm($carid,$brand,$model,$name,$email,$mobile,$startdate,$enddate,$location,$bookingstatus,$totalprice)
{
        $currentUser = $_COOKIE["username"];
        $con = getConnection();
        $sql = "INSERT INTO carbooking (Username, Fullname, Email, Mobile, CarID, Brand, Model, StartDate, EndDate, Location, TotalPrice, Status) 
        VALUES ('{$currentUser}', '{$name}', '{$email}', '{$mobile}', '{$carid}', '{$brand}', '{$model}', '{$startdate}', '{$enddate}', '{$location}', '{$totalprice}', '{$bookingstatus}')";
        $result = mysqli_query($con, $sql);
        if ($result) {
            return true;
        } else {
