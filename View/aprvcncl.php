...
<head>
    ...
    <link rel="stylesheet" href="../css/aprvcnclstyle.css">
    <title>Approve Cancellation</title>
</head>


<body>
    <fieldset>
        <legend>Car Cancellation Details</legend>
        <?php
        $carId = $_GET['id'] ?? '';
        if (!$carId) {
            ...
            if (mysqli_num_rows($result) > 0) {
                ...
                if ($row['Status'] === 'Cancel?') {
                    echo "<form action='../controller/aprvcnclval.php' method='post'>";
                    echo "<input type='hidden' name='carid' value='" . $row['CarID'] . "'>";
                    echo "<input type='submit' name='approve' value='Yes'>";
                    echo "</form>";
                    echo "<a href='../view/carbooking.php'>No</a>";
                } else {
                    echo "<p>Sorry, the cancellation is not available for approval.</p>";
